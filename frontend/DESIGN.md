...
## Adaptation Notes for This Project (Web / Next.js / daisyUI)

This design system originates from a WeChat mini-program (rpx units, `view`/`text` primitives). For the chatbot demo (Next.js + Tailwind + daisyUI, standard web DOM), apply it as follows:

- **Units**: `rpx` -> `rem`, using the mini-program's common 2rpx = 1px convention, then /16 to get rem. So the spacing/radius/typography scale becomes: xs 4px/0.25rem, sm 8px/0.5rem, md 12px/0.75rem, lg 16px/1rem, xl 24px/1.5rem; radius sm 4px, md 8px, lg 12px, xl 16px, round 999px.
- **Palette, shadows, motion timing, do's/don'ts**: apply as-is — these are platform-agnostic.
- **Component patterns actually used here**: buttons (pill, gold CTA), cards (white bg, colored left accent bar), section titles (emoji + dashed border), colored glow shadows, bounce/scale micro-interactions.
- **Component patterns NOT used here** (Flip7-game-specific, not applicable to a customer-support chat widget): Logo/ribbon banner, counter buttons, BOOM button, Flip7 button, victory rankings, confetti, crown bounce.
